...
class Machine:
    ...
    def _run(self, cmd, raise_error=True, env=None):
        """
    ...
    def _run_blocking(self, cmd, raise_error=True, env=None):
        """
        Run a docker-machine command, optionally raise error if error code != 0
        Args:
            cmd (List[str]): a list of the docker-machine command with the arguments to run
    ...
    def create(self, name, driver='virtualbox', blocking=True,
        ...
        if blocking:
            stdout, stderr, errorcode = self._run_blocking(cmd, env=env)
        else:
            stdout, stderr, errorcode = self._run(cmd, env=env)
